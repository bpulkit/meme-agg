...
describe("WebSocket tests", () => {
  ...
  test("socket subscribe receives initial_data", (done) => {
    ...
    clientA.on("initial_data", (data: any) => {
      try {
  ...
  test("socket receives price_update_batch events", (done) => {
    ...
    // wait for initial_data then listen for batch
    clientB.on("initial_data", (_data: any) => {
      clientB.on("price_update_batch", (batch: any) => {
        try {
          expect(Array.isArray(batch)).toBeTruthy();
          // batch elements have token_address field
