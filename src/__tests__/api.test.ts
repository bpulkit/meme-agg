...
beforeAll(async () => {
  // ensure server is running (we assume you start manually with MOCK_API=1 node dist/index.js)
  // If you want tests to spawn server, that is more complex; for now tests assume server running.
});
...
describe("REST API - basic", () => {
  ...
  test("sort by price_change period 1h", async () => {
    ...
    expect(r.status).toBe(200);
    const items = r.data.items;
    // check descending order by price_1hr_change
  ...
  test("pagination nextCursor works", async () => {
    ...
    if (next) {
      ...
      // if no nextCursor, at least ensure items length <= limit
